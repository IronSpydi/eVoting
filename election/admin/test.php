<?php 
	...
	if (isset($_REQUEST['submit'])) {
		if (($_REQUEST['firstname']=="")||($_REQUEST['lastname']=="")||($_REQUEST['tele']=="")||($_REQUEST['bday']=="")||($_REQUEST['gedner']=="")||($_REQUEST['politicalparty']=="")||($_REQUEST['state']=="")||($_REQUEST['city']=="")||($_REQUEST['ward']=="")||($_REQUEST['area']=="")) {
			
			echo '<script type="text/javascript">alert("fill all field");</script>';
			
		}else{
			$sql="INSERT into test (firstname,lastname,bday,tele,gedner,politicalparty,state,city,ward,area) VALUES (?,?,?,?,?,?,?,?,?,?)";
			...
			$result->execute(array($fn,$ln,$bday,$tele,$gedner,$politicalparty,$state,$city,$ward,$area));
	 ...
	 if(isset($_REQUEST['delete'])){
      ...
      if(isset($_REQUEST['up'])){
        $sql="UPDATE test SET firstname=?,lastname=?,bday=?,gedner=?,tele=?,politicalparty=?,area=?,state=?,city=?,ward=? WHERE id=?";
          $id=$_REQUEST['id'];
         ...
         $area=$_REQUEST['area'];
          ...
          $result=$con->prepare($sql);
          $result->execute(array($firstname,$lastname,$bday,$gedner,$tele,$politicalparty,$area,$state,$city,$ward,$id));
...
<ul class="collapsible popout">
 ...
 <form action="test.php" method="POST">
 	...
 	<label>bday</label><br>
 	<input type="date" name="bday" value="<?php if(isset($row['bday'])){ echo $row["bday"];}?>"><br>
