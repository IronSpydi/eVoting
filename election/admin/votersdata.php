<?php 
	...
	if (isset($_REQUEST['submit'])) {
		if (($_REQUEST['firstname']=="")||($_REQUEST['lastname']=="")||($_REQUEST['tele']=="")||($_REQUEST['DOB']=="")||($_REQUEST['gedner']=="")||($_REQUEST['state']=="")||($_REQUEST['city']=="")||($_REQUEST['ward']=="")||($_REQUEST['area']=="")||($_REQUEST['email']=="")) {
			
			echo '<script type="text/javascript">alert("fill all field");</script>';
			
		}else{
			$sql="INSERT into votersdata (firstname,lastname,DOB,tele,gedner,state,city,ward,area,email) VALUES (?,?,?,?,?,?,?,?,?,?)";
		...
		$email=$_REQUEST['email'];
			...
			$result->execute(array($fn,$ln,$DOB,$tele,$gedner,$state,$city,$ward,$area,$email));
	 ...
	 if(isset($_REQUEST['delete'])){
        $id=$_REQUEST['id'];
        $sql="DELETE FROM votersdata WHERE id=$id";
        $result=$con->prepare($sql);
        $result->execute();
      }
      ...
      if(isset($_REQUEST['up'])){
         ...
         $area=$_REQUEST['area'];
          ...
          $ward=$_REQUEST['ward'];
           $email=$_REQUEST['email'];
          $sql="UPDATE votersdata SET firstname='$firstname',lastname='$lastname',DOB='$DOB',area='$area' ,tele='$tele',state='$state',gedner='$gedner',area='$area',city='$city',ward='$ward',email='$email' WHERE id=$id";
         
          $result=$con->prepare($sql);
          $result->execute(array($firstname,$lastname,$DOB,$tele,$state,$city,$ward,$area,$email));
      ...
      <div class="collapsible-body">
 			<?php
                if(isset($_REQUEST['edit'])){
                  $sql="SELECT * FROM votersdata WHERE id={$_REQUEST['id']}";
                  $result=$con->prepare($sql);
                  $result->execute();
                  //print_r($result);
              ...
              ?>
 <form action="votersdata.php" method="POST">
        ...
        echo  '<table class="table">';
          ...
          while ($row = $result->fetch(PDO::FETCH_ASSOC)){
            ...
            echo "<tr>";    
              ...
              echo '<td>'.'<form action="votersdata.php" method="POST"><input type="hidden" name="id" value='.$row['id'].'><input type="submit" class="btn" name="edit" value="Edit"></form>'.'</td>';
              echo "<td>".'<form action="votersdata.php" method="POST"><input name="id" type="hidden" value='.$row['id'].'><input type="submit" class="btn" name="delete" value="DELETE"></form>'."</td>";
